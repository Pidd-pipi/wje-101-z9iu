...
func (s *UserService) Register(username, email, password, bio string) (*model.User, string, error) {
	hash, err := bcrypt.GenerateFromPassword([]byte(password), bcrypt.DefaultCost)
	if err != nil {
		return nil, "", fmt.Errorf("user register: %w", err)
	}
	u := &model.User{Username: username, Email: email, PasswordHash: string(hash), Bio: bio, Role: constants.RoleUser}
...
func (s *UserService) UpdateProfile(id uint, bio, avatar string) (*model.User, error) {
