...
func registerUserRoutes(v1 *gin.RouterGroup, cfg *config.Config, h *handler.UserHandler, bh *handler.BeanHandler, fh *handler.FollowHandler, limiter *middleware.RateLimiter) {
	users := v1.Group("/users")
	users.POST("/register", limiter.Limit(), h.Register)
	users.POST("/login", limiter.Limit(), h.Login)
	users.GET("/:id/profile", h.Profile)
	users.GET("/:id/beans", bh.UserBeans)
