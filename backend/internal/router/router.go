...
func Setup(cfg *config.Config, db *gorm.DB, logger *slog.Logger) *gin.Engine {
	...
	beanListRepo := repository.NewUserBeanListRepository(db)

	userService := service.NewUserService(userRepo, logger, cfg)
	noteService := service.NewNoteService(noteRepo, beanRepo, beanListRepo, logger)
	recipeService := service.NewRecipeService(recipeRepo, logger)
	beanService := service.NewBeanService(beanRepo, noteRepo, beanListRepo, logger)
	beanListService := service.NewUserBeanListService(beanListRepo, beanRepo, noteRepo, logger)
	...
	beanHandler := handler.NewBeanHandler(beanService, beanListService, logger)
	...
	v1 := r.Group("/api/v1")
	{
		registerUserRoutes(v1, cfg, userHandler, beanHandler, followHandler, limiter)
